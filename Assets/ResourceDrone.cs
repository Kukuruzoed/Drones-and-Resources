...
public class ResourceDrone : MonoBehaviour
{
    ...
    public Vector3 GetAvoidanceForce(Vector3 position, GameObject currentDrone)
    {
        ...
        foreach (var drone in drones)
        {
            ...
            {
                // Чем ближе объект, тем больше сила отталкивания (обратнопропорциональна квадрату расстояния)
                Vector3 away = (position - drone.transform.position).normalized;
                float strength = 1f / (dist * dist); // экспоненциально растёт при приближении
                force += away * strength;
            }
        }
